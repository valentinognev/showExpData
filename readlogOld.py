...
def decode(filName : str, printOutput : bool = False):
    # read file as binary
    ...
    for ii in range(0, setWidth[0]):
        ...
        idx += 1
    if printOutput : print("[CRC] of file header:", end="")
    crcVal = crc32(filCon[0:idx+4]) & 0xffffffff
    crcErrors = 0
    if (crcVal == 0xffffffff):
        if printOutput : print("\tOK\t["+hex(crcVal)+"]")
    else:
    ...
    while (offset < len(filCon)):
        ...
        for ii in range(setNumber[0]):
            setCon[idx:idx+setWidth[0]
                   ] = np.array(struct.unpack(fmtStr, filCon[offset:setBytes+offset]))
            offset += setBytes
            idx += setWidth[0]
        crcVal = crc32(
            filCon[offset-setBytes*setNumber[0]-1:offset+4]) & 0xffffffff
        if printOutput : print("[CRC] of data set:", end="")
        if (crcVal == 0xffffffff):
            if printOutput : print("\tOK\t["+hex(crcVal)+"]")
        else:
            print("\tERROR\t["+hex(crcVal)+"]")
        ...
        offset += 4
    if (not crcErrors):
        if printOutput : print("[CRC] no errors occurred:\tOK")
    else:
